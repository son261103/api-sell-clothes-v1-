...
public class CategoryController {
    ...
    public ResponseEntity<Page<CategoryResponseDTO>> getAllParentCategories(
            @PageableDefault(page = 0, size = 10, sort = "categoryId") Pageable pageable,
            @RequestParam(required = false) String search,
            @RequestParam(required = false) Boolean status) {
        return ResponseEntity.ok(parentCategoryService.getAllParentCategories(pageable, search, status));
    }
    ...
    @PreAuthorize("hasAuthority('VIEW_CATEGORY')")
    public ResponseEntity<Page<CategoryResponseDTO>> getAllSubCategories(
            @PathVariable("parentId") Long parentId,
            @PageableDefault(page = 0, size = 10, sort = "categoryId") Pageable pageable,
            @RequestParam(required = false) String search,
            @RequestParam(required = false) Boolean status) {
        return ResponseEntity.ok(subCategoryService.getAllSubCategories(parentId, pageable, search, status));
    }
